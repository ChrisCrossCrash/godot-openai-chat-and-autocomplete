...
const PROMPT_PREFIX = """
**SYSTEM PROMPT: GodotGemini - GDScript 4.x Expert Coding Assistant**
...
func get_completion(_prompt, _suffix):
	...
	var tempUrl = "https://generativelanguage.googleapis.com/v1beta/"+model+":generateContent?key="+api_key
	print_rich("[b]get_completion[/b] - ",tempUrl)
	var error = http_request.request(tempUrl, headers, HTTPClient.METHOD_POST, json_body)
	if error != OK:
...
func on_request_completed(result, response_code, headers, body, pre, post, http_request):
	...
	var response = json
	if !response:
		push_warning("Error, no response from api", response_code,body.get_string_from_utf8())
		emit_signal("completion_error", response)
		return
	if response.has("error"):
		var code = response.error.code
		var message = response.error.message
		print_rich("[b]on_request_completed[/b] - Error while calling completion request: ", message)
		emit_signal("completion_error", response)
		return
	else:
		...
		if !candidates.has("content"):
			emit_signal("completion_error", response)
			return
		if !candidates.content.has("parts"):
			emit_signal("completion_error", response)
			return
		var completion = candidates.content.parts[0].text
		chat_history.push_back(candidates.content)
...
func chat_message(newText:String):
	...
	var json_body = JSON.stringify(body)
	var error = http_request.request("https://generativelanguage.googleapis.com/v1beta/"+model+":generateContent?key="+api_key, headers, HTTPClient.METHOD_POST, json_body)
	if error != OK:
		emit_signal("completion_error", null)
...
func on_chat_complete(result, response_code, headers, body):
	...
	if response.has("error"):
		var code = response.error.code
		var message = response.error.message
		emit_signal("chat_received", "Error with message: " + message)
		return
	else:
		if !response.has("candidates"):
			emit_signal("completion_error", response)
		...
		if !candidates.content.has("parts"):
			...
			return
		var completion = candidates.content.parts[0].text
		chat_history.push_front(candidates.content)
